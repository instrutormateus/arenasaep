...
export async function analyzeQuestionPages(env, draft, pages) {
  ...
  for (const page of pages.slice(0, 5)) {
    const messages = [
      {
        role: "system",
        content: "Você é um transcritor técnico de avaliações SAEP. Preserve literalmente as palavras, números, unidades, símbolos e pontuação do conteúdo visível. Remova somente quebras de linha causadas pela diagramação do PDF: una trechos da mesma frase em um único parágrafo e preserve quebras apenas em listas ou itens numerados. As questões podem ter exatamente 2, 4 ou 5 alternativas. Uma alternativa pode ser composta somente por imagem, circuito, tabela, símbolo ou diagrama; nesse caso mantenha o texto vazio e associe uma caixa visual ao alvo correto. O título FOLHA DE RESPOSTA encerra a seção de questões e nunca deve ser interpretado como questão. A seção GABARITO é uma tabela separada; não invente respostas e preserve o gabarito já associado ao rascunho pelo sistema.",
      },
      {
        role: "user",
        content: `Analise a página ${page.page} relacionada à questão ${draft.id}. O rascunho extraído pelo PDF.js está abaixo. Não resuma, não corrija erros do original e não invente conteúdo. Não devolva quebras de linha no meio de frases; una linhas de diagramação e mantenha novas linhas apenas para listas ou itens independentes. Reconheça exatamente ${draft.quantidadeAlternativas || "2, 4 ou 5"} alternativas, incluindo alternativas exclusivamente visuais. Para figuras, retorne caixas normalizadas de 0 a 1000 e use target=question para imagem do enunciado ou target=A/B/C/D/E para imagem de alternativa. Ignore logotipo, cabeçalho, rodapé, número da página, FOLHA DE RESPOSTA e GABARITO. O campo correta, quando presente no rascunho, veio da tabela oficial de gabarito e não deve ser modificado.\n\n${JSON.stringify(draft, null, 2)}`,
      },
    ];
